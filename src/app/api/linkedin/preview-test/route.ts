// TEST VERSION - LinkedIn Preview API without authentication
...
export async function POST(request: NextRequest) {
  try {
    // TESTING: Skip authentication check
    console.log('⚠️  TEST MODE: Skipping authentication')
...
export async function GET() {
  return NextResponse.json({
    message: 'LinkedIn Preview TEST API is running (NO AUTH)',
    warning: 'This is a test endpoint without authentication. Do not use in production!',
    endpoints: {
      POST: '/api/linkedin/preview-test - Fetch LinkedIn profile preview data (NO AUTH)'
    }
  })
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const process: Process = {
      ...
      ownerName: (session.user as any).name || `${(session.user as any).firstName} ${(session.user as any).lastName}`,
      ...
      createdByName: (session.user as any).name || `${(session.user as any).firstName} ${(session.user as any).lastName}`,
